...
sys.path.append("Utils/Maubat_Utils")
...
def buscaIce(templateg, rsp=100, alg=1, minseg=10, maxseg=np.inf):
    ...
    xlin, xcol = np.where(
        rank.median(convert(templateg, 0, 255, np.uint8), disk(4)) > 5
    )
    ...
    for ill in range(0, int(stepll * frames), stepll):
        ...
        for jll in range(0, int(stepcc * frames), stepcc):
            ...
            if alg == 1:
                ...
                segmentos = seg.segFelzenszwalb(
                    sub_templateg, 12, 30, 0, 3
                )  # 19 30 0 3
            ...
            for segmentoAtual in regions:
                ...
                if ar_seg >= minseg and ar_seg < maxseg:
                    ...
                    if per_seg > 0:
                        ...
                        pick1 = cv2.bilateralFilter(
                            imresize(pick1, (32, 32), interp="bilinear"), 3, 3, 3
                        ).astype("float")
                        pickback = cv2.bilateralFilter(
                            imresize(pickback, (32, 32), interp="bilinear"), 3, 3, 3
                        ).astype("float")
                        ...
                        if avgpick > 30 and dstpb > dsplim:  # 30 20
                            ...
                            segOutput.eq_diameter.append(
                                segmentoAtual["equivalent_diameter"]
                            )
                            ...
                            if rsp < 100:
                                ...
                                lp = int((lp * 100.0) / rsp)
                                cp = int((cp * 100.0) / rsp)
                                pick2morpho = imresize(
                                    pick2morpho, (lp, cp), interp="bilinear"
                                )
